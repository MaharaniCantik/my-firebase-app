...
function listenMenus(callback) {
    ...
    unsubMenus = onSnapshot(collection(db, 'menus'), (snapshot) => {
        menusData = [];
        ...
        container.innerHTML += `
            <div class="p-3 bg-gray-50 rounded-lg border border-gray-200 text-xs">
                <div class="flex justify-between font-bold text-gray-700">
                    ...
                    <span class="px-1.5 py-0.5 rounded text-[10px] ${o.status === 'Selesai' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}">${o.status}</span>
                </div>
